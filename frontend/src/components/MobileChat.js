...
const MobileChat = () => {
    ...
    const getMobileProgrammers = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await fetch('/api/programmers/mobile'); // Obtener programadores móviles
            if (!response.ok) {
    ...
    const handleSendMessage = useCallback(async (messageText) => {
        if (messageText.trim() && selectedProgrammer) {
            const chatId = 'chat_id'; // Reemplaza con el ID real del chat
            const senderId = 'user_id'; // Reemplaza con el ID real del usuario
            const senderType = 'user'; // Cambia a 'programmer' si corresponde
        ...
        <div className="flex h-screen bg-gray-100">
            <div className="w-full bg-white shadow-lg overflow-y-auto">
                <div className="p-4 border-b">
                    <h2 className="text-xl font-bold">Programadores Móviles</h2>
                </div>
